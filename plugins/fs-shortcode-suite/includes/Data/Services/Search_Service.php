<?php
declare(strict_types=1);

namespace FS\ShortcodeSuite\Data\Services;

use WP_Query;

defined('ABSPATH') || exit;

final class Search_Service
{
    private const LIMIT = 10;
    
    private function variant_is_valid(
        int $variant_id,
        int $product_id,
        array $filters,
        float $price): bool 
    {
    
        /*
         * =========================================
         *  PRODUCT TAXONOMIES
         * =========================================
         */
    
        if (!empty($filters['marca'])) {
            if (!has_term($filters['marca'], 'fs_marca', $product_id)) {
                return false;
            }
        }
    
        if (!empty($filters['genero'])) {
            if (!has_term($filters['genero'], 'fs_genero', $product_id)) {
                return false;
            }
        }
    
        /*
         * =========================================
         *  VARIANT TAXONOMIES
         * =========================================
         */
    
        foreach ([
            'color'      => 'fs_color',
            'superficie' => 'fs_superficie',
        ] as $key => $taxonomy) {
    
            if (!empty($filters[$key])) {
    
                $terms = wp_get_post_terms($variant_id, $taxonomy, ['fields' => 'slugs']);
    
                if (!in_array($filters[$key], $terms, true)) {
                    return false;
                }
            }
        }
    
        /*
         * =========================================
         *  PRICE RANGE
         * =========================================
         */
    
        if (!empty($filters['price_min']) && $price < (float) $filters['price_min']) {
            return false;
        }
    
        if (!empty($filters['price_max']) && $price > (float) $filters['price_max']) {
            return false;
        }
    
        return true;
    }

    public function search(string $query = '', array $filters = []): array
    {
        $query = trim($query);
    
        // ✅ Normaliza filtros y evita "Undefined array key"
        $filters = is_array($filters) ? $filters : [];
        
        // Si vienen como array tipo { marca: {skechers:"Skechers"} } o { talla: [37] }
        foreach (['marca', 'genero', 'color', 'superficie', 'talla'] as $key) {
            if (!isset($filters[$key])) {
                continue;
            }
        
            if (is_array($filters[$key])) {
                // Si es array numérico (ej: [37]) pillamos el primero
                $first = reset($filters[$key]);
                // Si es array asociativo (ej: ['skechers' => 'Skechers']) pillamos la primera key
                $filters[$key] = is_string(key($filters[$key])) ? (string) key($filters[$key]) : (string) $first;
            } else {
                $filters[$key] = (string) $filters[$key];
            }
        }
        
        /*
         * ============================================================
         *  0️⃣ NORMALIZAR FILTROS (frontend puede enviar arrays)
         * ============================================================
         */
        foreach (['marca', 'genero', 'color', 'superficie', 'talla'] as $key) {
            if (isset($filters[$key]) && is_array($filters[$key])) {
                $filters[$key] = array_key_first($filters[$key]);
            }
        }
    
        // Eliminar filtros vacíos reales
        $filters = array_filter($filters, function ($v) {
            return $v !== null && $v !== '' && $v !== [];
        });
    
        /*
         * ============================================================
         *  1️⃣ SI NO HAY QUERY NI FILTROS → SOLO DATASET DE FILTROS
         * ============================================================
         */
        if ($query === '' && empty($filters)) {
    
            $all_products_query = new WP_Query([
                'post_type'      => 'fs_producto',
                'post_status'    => 'publish',
                'fields'         => 'ids',
                'posts_per_page' => -1,
                'no_found_rows'  => true,
            ]);
    
            $all_ids = $all_products_query->posts ?: [];
    
            return [
                'products' => [],
                'filters'  => $this->build_filters_dataset($all_ids),
                'has_more' => false,
            ];
        }
    
        /*
         * ============================================================
         *  2️⃣ QUERY LIMITADA (LIMIT + 1 para detectar has_more)
         * ============================================================
         */
        $limited_args = $this->build_query_args($query, $filters);
        $limited_args['posts_per_page'] = self::LIMIT + 1;
    
        $limited_query = new WP_Query($limited_args);
        $product_ids   = $limited_query->posts ?: [];
    
        $has_more = count($product_ids) > self::LIMIT;
    
        if ($has_more) {
            $product_ids = array_slice($product_ids, 0, self::LIMIT);
        }
    
        /*
         * ============================================================
         *  3️⃣ DATASET COMPLETO PARA FACETS (sin límite)
         * ============================================================
         */
        $facet_args = $this->build_query_args($query, $filters);
        $facet_args['posts_per_page'] = -1;
    
        $facet_query = new WP_Query($facet_args);
        $facet_ids   = $facet_query->posts ?: [];
    
        /*
         * ============================================================
         *  4️⃣ BUILD PRODUCTS (variant-driven real)
         * ============================================================
         */
        $result_products = $this->build_products($product_ids, $filters);
    
        // Muy importante: solo usar productos realmente válidos
        $valid_product_ids = array_column($result_products, 'id');
    
        /*
         * ============================================================
         *  5️⃣ RESPONSE FINAL
         * ============================================================
         */
        return [
            'products' => $result_products,
            'filters' => $this->build_filters_dataset($valid_product_ids, $filters),
            'has_more' => $has_more,
        ];
    }

    /* ============================================================
     *  BUILD PRODUCT QUERY
     * ============================================================ */
    
    private function build_query_args(string $query, array $filters): array
    {
        return $this->build_variant_driven_query($query, $filters);
    }
    
    private function build_variant_driven_query(string $query, array $filters): array
{
    $query = trim((string) $query);

    /*
     * ============================================================
     * 0️⃣ GUARD CLAUSE
     * ============================================================
     */
    $hasAny =
        $query !== '' ||
        !empty($filters['marca']) ||
        !empty($filters['genero']) ||
        !empty($filters['color']) ||
        !empty($filters['superficie']) ||
        !empty($filters['talla']) ||
        !empty($filters['price_min']) ||
        !empty($filters['price_max']);

    if (!$hasAny) {
        return [
            'post_type' => 'fs_producto',
            'post__in'  => [0],
        ];
    }

    /*
     * ============================================================
     * 1️⃣ OFERTAS (STOCK + PRECIO + TALLA)
     * ============================================================
     */

    $offer_meta_query = [
        'relation' => 'AND',
        [
            'key'   => 'fs_in_stock',
            'value' => '1',
        ],
    ];

    // Precio max
    if (!empty($filters['price_max'])) {
        $offer_meta_query[] = [
            'key'     => 'fs_price',
            'value'   => (float) $filters['price_max'],
            'compare' => '<=',
            'type'    => 'NUMERIC',
        ];
    }

    // Precio min
    if (!empty($filters['price_min'])) {
        $offer_meta_query[] = [
            'key'     => 'fs_price',
            'value'   => (float) $filters['price_min'],
            'compare' => '>=',
            'type'    => 'NUMERIC',
        ];
    }

    // Talla
    if (!empty($filters['talla'])) {
        $talla = str_replace('.', '-', (string) $filters['talla']);

        $offer_meta_query[] = [
            'key'     => 'fs_size_eu',
            'value'   => $talla,
            'compare' => '=',
        ];
    }

    $offer_ids = get_posts([
        'post_type'              => 'fs_oferta',
        'post_status'            => 'publish',
        'fields'                 => 'ids',
        'posts_per_page'         => -1,
        'no_found_rows'          => true,
        'meta_query'             => $offer_meta_query,
        'update_post_meta_cache' => true,   // usamos meta luego
        'update_post_term_cache' => false,
    ]);

    if (empty($offer_ids)) {
        return [
            'post_type' => 'fs_producto',
            'post__in'  => [0],
        ];
    }

    /*
     * ============================================================
     * 2️⃣ OFERTAS → VARIANT_GLOBAL_ID
     * ============================================================
     */

    $variant_global_ids = [];

    foreach ($offer_ids as $offer_id) {
        $global_id = get_post_meta((int) $offer_id, 'fs_variant_id', true);
        if ($global_id !== '') {
            $variant_global_ids[] = (string) $global_id;
        }
    }

    $variant_global_ids = array_values(array_unique($variant_global_ids));

    if (empty($variant_global_ids)) {
        return [
            'post_type' => 'fs_producto',
            'post__in'  => [0],
        ];
    }

    /*
     * ============================================================
     * 3️⃣ VARIANTES (COLOR / SUPERFICIE)
     * ============================================================
     */

    $variant_tax_query = ['relation' => 'AND'];

    foreach ([
        'color'      => 'fs_color',
        'superficie' => 'fs_superficie',
    ] as $key => $taxonomy) {

        if (!empty($filters[$key])) {
            $variant_tax_query[] = [
                'taxonomy' => $taxonomy,
                'field'    => 'slug',
                'terms'    => sanitize_title((string) $filters[$key]),
            ];
        }
    }

    $variant_args = [
        'post_type'              => 'fs_variante',
        'post_status'            => 'publish',
        'fields'                 => 'ids',
        'posts_per_page'         => -1,
        'no_found_rows'          => true,
        'meta_query'             => [
            [
                'key'     => 'fs_variant_id',
                'value'   => $variant_global_ids,
                'compare' => 'IN',
            ],
        ],
        'update_post_meta_cache' => false,
        'update_post_term_cache' => true,
    ];

    if (count($variant_tax_query) > 1) {
        $variant_args['tax_query'] = $variant_tax_query;
    }

    $variant_ids = get_posts($variant_args);

    if (empty($variant_ids)) {
        return [
            'post_type' => 'fs_producto',
            'post__in'  => [0],
        ];
    }

    /*
     * ============================================================
     * 4️⃣ VARIANTES → PRODUCTOS
     * ============================================================
     */

    $product_ids = array_unique(
        array_filter(
            array_map('intval', array_map('wp_get_post_parent_id', $variant_ids))
        )
    );

    if (empty($product_ids)) {
        return [
            'post_type' => 'fs_producto',
            'post__in'  => [0],
        ];
    }

    /*
     * ============================================================
     * 5️⃣ FILTROS DE PRODUCTO (MARCA / GÉNERO) + QUERY
     * ============================================================
     */

    $product_tax_query = ['relation' => 'AND'];

    if (!empty($filters['marca'])) {
        $product_tax_query[] = [
            'taxonomy' => 'fs_marca',
            'field'    => 'slug',
            'terms'    => sanitize_title((string) $filters['marca']),
        ];
    }

    if (!empty($filters['genero'])) {
        $product_tax_query[] = [
            'taxonomy' => 'fs_genero',
            'field'    => 'slug',
            'terms'    => sanitize_title((string) $filters['genero']),
        ];
    }

    $out = [
        'post_type'              => 'fs_producto',
        'post_status'            => 'publish',
        'post__in'               => $product_ids,
        'orderby'                => 'post__in',
        'fields'                 => 'ids',
        'no_found_rows'          => true,
        'update_post_meta_cache' => false,
        'update_post_term_cache' => false,
    ];

    if (count($product_tax_query) > 1) {
        $out['tax_query'] = $product_tax_query;
    }

    if ($query !== '') {
        $out['meta_query'] = [
            [
                'key'     => 'fs_model_signature',
                'value'   => $query,
                'compare' => 'LIKE',
            ],
        ];
    }

    return $out;
}
    
    /* ============================================================
     *  BUILD PRODUCTS (PRECIO REAL DESDE OFERTAS)
     * ============================================================ */

    private function build_products(array $ids, array $filters = []): array
    {
        if (empty($ids)) {
            return [];
        }
    
        $ids = array_map('intval', $ids);
    
        /*
         * ============================================================
         * 1️⃣ VARIANTES MASIVAS (CON CACHE ACTIVADA)
         * ============================================================
         */
    
        $variant_ids = get_posts([
            'post_type'              => 'fs_variante',
            'post_status'            => 'publish',
            'fields'                 => 'ids',
            'posts_per_page'         => -1,
            'no_found_rows'          => true,
            'post_parent__in'        => $ids,
            'update_post_meta_cache' => true,   // 🔥 importante
            'update_post_term_cache' => true,   // 🔥 importante
        ]);
    
        if (empty($variant_ids)) {
            return [];
        }
    
        $variants_by_product = [];
        $variant_global_ids  = [];
    
        foreach ($variant_ids as $variant_id) {
    
            $variant_id = (int) $variant_id;
            $product_id = (int) wp_get_post_parent_id($variant_id);
    
            $variants_by_product[$product_id][] = $variant_id;
    
            $global_id = (string) get_post_meta($variant_id, 'fs_variant_id', true);
            if ($global_id !== '') {
                $variant_global_ids[$variant_id] = $global_id;
            }
        }
    
        if (empty($variant_global_ids)) {
            return [];
        }
    
        /*
         * ============================================================
         * 2️⃣ MAPA INVERSO GLOBAL_ID → VARIANT_ID (O(1))
         * ============================================================
         */
    
        $global_to_variant = array_flip($variant_global_ids);
    
        /*
         * ============================================================
         * 3️⃣ OFERTAS MASIVAS (CON CACHE)
         * ============================================================
         */
    
        $offer_ids = get_posts([
            'post_type'              => 'fs_oferta',
            'post_status'            => 'publish',
            'fields'                 => 'ids',
            'posts_per_page'         => -1,
            'no_found_rows'          => true,
            'meta_query'             => [
                [
                    'key'     => 'fs_variant_id',
                    'value'   => array_values($variant_global_ids),
                    'compare' => 'IN',
                ]
            ],
            'update_post_meta_cache' => true,  // 🔥
            'update_post_term_cache' => false,
        ]);
    
        if (empty($offer_ids)) {
            return [];
        }
    
        /*
         * ============================================================
         * 4️⃣ AGRUPAR OFERTAS POR VARIANTE (SIN array_search)
         * ============================================================
         */
    
        $offers_by_variant = [];
    
        foreach ($offer_ids as $offer_id) {
    
            $offer_id = (int) $offer_id;
    
            $global_id = get_post_meta($offer_id, 'fs_variant_id', true);
    
            if (!isset($global_to_variant[$global_id])) {
                continue;
            }
    
            $variant_id = $global_to_variant[$global_id];
    
            $offers_by_variant[$variant_id][] = $offer_id;
        }
    
        /*
         * ============================================================
         * 5️⃣ CONSTRUIR OUTPUT
         * ============================================================
         */
    
        $out = [];
    
        foreach ($ids as $product_id) {
    
            $product_id = (int) $product_id;
    
            $variant_list = $variants_by_product[$product_id] ?? [];
    
            if (empty($variant_list)) {
                continue;
            }
    
            $best_price = null;
            $colors = [];
            $sizes  = [];
    
            foreach ($variant_list as $variant_id) {
    
                /*
                 * 🔥 get_the_terms usa cache si activamos update_post_term_cache
                 */
                $color_terms = get_the_terms($variant_id, 'fs_color');
    
                if (!empty($color_terms) && !is_wp_error($color_terms)) {
                    foreach ($color_terms as $term) {
                        $colors[$term->slug] = true;
                    }
                }
    
                $offer_list = $offers_by_variant[$variant_id] ?? [];
    
                foreach ($offer_list as $offer_id) {
    
                    $in_stock = get_post_meta($offer_id, 'fs_in_stock', true);
    
                    if (in_array($in_stock, ['0', 0, false, 'false', '', null], true)) {
                        continue;
                    }
    
                    $size = get_post_meta($offer_id, 'fs_size_eu', true);
                    if ($size !== '') {
                        $sizes[(string) $size] = true;
                    }
    
                    $price_raw      = (string) get_post_meta($offer_id, 'fs_price', true);
                    $price_sale_raw = (string) get_post_meta($offer_id, 'fs_price_sale', true);
    
                    $price      = (float) str_replace(',', '.', $price_raw);
                    $price_sale = (float) str_replace(',', '.', $price_sale_raw);
    
                    $final = ($price_sale > 0) ? $price_sale : $price;
    
                    if ($final <= 0) {
                        continue;
                    }
    
                    if ($best_price === null || $final < $best_price) {
                        $best_price = $final;
                    }
                }
            }
    
            if ($best_price === null) {
                continue;
            }
    
            $out[] = [
                'id'           => $product_id,
                'name'         => get_the_title($product_id),
                'permalink'    => get_permalink($product_id),
                'image'        => (string) get_post_meta($product_id, 'fs_image_main_url', true),
                'price_from'   => round($best_price, 2),
                'brand'        => $this->get_brand($product_id),
                'colors_count' => count($colors),
                'colors'       => array_keys($colors),
                'sizes'        => array_keys($sizes),
            ];
        }
    
        return $out;
    }

    /* ============================================================
     *  BUILD FILTER DATASET (REAL: VARIANTE + OFERTA)
     * ============================================================ */
    
    private function variant_matches_filters(int $variant_id, array $filters): bool
    {
        foreach ([
            'color'      => 'fs_color',
            'superficie' => 'fs_superficie',
            'talla'      => 'fs_talla_eu',
        ] as $key => $taxonomy) {
    
            if (!empty($filters[$key])) {
    
                $terms = wp_get_post_terms($variant_id, $taxonomy, ['fields' => 'slugs']);
    
                if (empty($terms) || !in_array($filters[$key], $terms, true)) {
                    return false;
                }
            }
        }
    
        return true;
    }
    
    private function build_filters_dataset(array $product_ids, array $filters = []): array
    {
        if (empty($product_ids)) {
            return [
                'talla'      => [],
                'superficie' => [],
                'color'      => [],
                'marca'      => [],
                'genero'     => [],
                'price_min'  => null,
                'price_max'  => null,
            ];
        }
    
        $sizes     = [];
        $surfaces  = [];
        $colors    = [];
        $brands    = [];
        $generos   = [];
        $price_min = null;
        $price_max = null;
    
        /*
         * ============================================================
         * 1️⃣ PRODUCT_HASHES
         * ============================================================
         */
    
        $product_hashes = [];
    
        foreach ($product_ids as $product_id) {
            $hash = get_post_meta($product_id, 'fs_product_id', true);
            if ($hash !== '') {
                $product_hashes[] = $hash;
            }
        }
    
        $product_hashes = array_unique($product_hashes);
    
        if (empty($product_hashes)) {
            return [
                'talla'      => [],
                'superficie' => [],
                'color'      => [],
                'marca'      => [],
                'genero'     => [],
                'price_min'  => null,
                'price_max'  => null,
            ];
        }
    
        /*
         * ============================================================
         * 2️⃣ VARIANTES FILTRADAS (COLOR / SUPERFICIE)
         * ============================================================
         */
    
        $variant_tax_query = ['relation' => 'AND'];
    
        foreach ([
            'color'      => 'fs_color',
            'superficie' => 'fs_superficie',
        ] as $key => $taxonomy) {
    
            if (!empty($filters[$key])) {
                $variant_tax_query[] = [
                    'taxonomy' => $taxonomy,
                    'field'    => 'slug',
                    'terms'    => sanitize_title((string) $filters[$key]),
                ];
            }
        }
    
        $variant_ids = get_posts([
            'post_type'      => 'fs_variante',
            'post_status'    => 'publish',
            'fields'         => 'ids',
            'posts_per_page' => -1,
            'no_found_rows'  => true,
            'meta_query'     => [
                [
                    'key'     => 'fs_product_id',
                    'value'   => $product_hashes,
                    'compare' => 'IN',
                ]
            ],
            'tax_query'      => (count($variant_tax_query) > 1) ? $variant_tax_query : [],
        ]);
    
        if (empty($variant_ids)) {
            return [
                'talla'      => [],
                'superficie' => [],
                'color'      => [],
                'marca'      => [],
                'genero'     => [],
                'price_min'  => null,
                'price_max'  => null,
            ];
        }
    
        /*
         * ============================================================
         * 3️⃣ VARIANT_GLOBAL_IDS
         * ============================================================
         */
    
        $variant_global_ids = [];
    
        foreach ($variant_ids as $variant_id) {
            $gid = get_post_meta($variant_id, 'fs_variant_id', true);
            if ($gid !== '') {
                $variant_global_ids[] = $gid;
            }
        }
    
        $variant_global_ids = array_unique($variant_global_ids);
    
        if (empty($variant_global_ids)) {
            return [
                'talla'      => [],
                'superficie' => [],
                'color'      => [],
                'marca'      => [],
                'genero'     => [],
                'price_min'  => null,
                'price_max'  => null,
            ];
        }
    
        /*
         * ============================================================
         * 4️⃣ OFERTAS FILTRADAS (TALLA / PRECIO / STOCK)
         * ============================================================
         */
    
        $offer_meta_query = [
            'relation' => 'AND',
            [
                'key'     => 'fs_variant_id',
                'value'   => $variant_global_ids,
                'compare' => 'IN',
            ],
            [
                'key'   => 'fs_in_stock',
                'value' => '1',
            ],
        ];
    
        if (!empty($filters['talla'])) {
            $talla = str_replace('.', '-', (string) $filters['talla']);
            $offer_meta_query[] = [
                'key'     => 'fs_size_eu',
                'value'   => $talla,
                'compare' => '=',
            ];
        }
    
        if (!empty($filters['price_max'])) {
            $offer_meta_query[] = [
                'key'     => 'fs_price',
                'value'   => (float) $filters['price_max'],
                'compare' => '<=',
                'type'    => 'NUMERIC',
            ];
        }
    
        if (!empty($filters['price_min'])) {
            $offer_meta_query[] = [
                'key'     => 'fs_price',
                'value'   => (float) $filters['price_min'],
                'compare' => '>=',
                'type'    => 'NUMERIC',
            ];
        }
    
        $offer_ids = get_posts([
            'post_type'      => 'fs_oferta',
            'post_status'    => 'publish',
            'fields'         => 'ids',
            'posts_per_page' => -1,
            'no_found_rows'  => true,
            'meta_query'     => $offer_meta_query,
        ]);
    
        if (empty($offer_ids)) {
            return [
                'talla'      => [],
                'superficie' => [],
                'color'      => [],
                'marca'      => [],
                'genero'     => [],
                'price_min'  => null,
                'price_max'  => null,
            ];
        }
    
        /*
         * ============================================================
         * 5️⃣ CONSTRUIR DATASET DESDE OFERTAS VÁLIDAS
         * ============================================================
         */
    
        $valid_variant_ids = [];
        $valid_product_ids = [];
    
        foreach ($offer_ids as $offer_id) {
    
            $size = get_post_meta($offer_id, 'fs_size_eu', true);
            if ($size !== '') {
                $sizes[(string)$size] = true;
            }
    
            $price_raw      = get_post_meta($offer_id, 'fs_price', true);
            $price_sale_raw = get_post_meta($offer_id, 'fs_price_sale', true);
    
            $price      = (float) str_replace(',', '.', (string)$price_raw);
            $price_sale = (float) str_replace(',', '.', (string)$price_sale_raw);
    
            $final = ($price_sale > 0) ? $price_sale : $price;
    
            if ($final > 0) {
                $price_min = ($price_min === null) ? $final : min($price_min, $final);
                $price_max = ($price_max === null) ? $final : max($price_max, $final);
            }
    
            $variant_global_id = get_post_meta($offer_id, 'fs_variant_id', true);
    
            $variant_id = get_posts([
                'post_type'  => 'fs_variante',
                'meta_query' => [
                    [
                        'key'   => 'fs_variant_id',
                        'value' => $variant_global_id,
                    ]
                ],
                'fields'     => 'ids',
                'numberposts'=> 1,
            ]);
    
            if (!empty($variant_id)) {
    
                $variant_id = $variant_id[0];
                $valid_variant_ids[] = $variant_id;
    
                $product_id = wp_get_post_parent_id($variant_id);
                $valid_product_ids[] = $product_id;
            }
        }
    
        $valid_variant_ids = array_unique($valid_variant_ids);
        $valid_product_ids = array_unique($valid_product_ids);
    
        /*
         * ============================================================
         * 6️⃣ COLOR / SUPERFICIE DESDE VARIANTES VÁLIDAS
         * ============================================================
         */
    
        foreach ($valid_variant_ids as $variant_id) {
    
            $surface_terms = get_the_terms($variant_id, 'fs_superficie');
            if (!empty($surface_terms)) {
                foreach ($surface_terms as $term) {
                    $surfaces[$term->slug] = true;
                }
            }
    
            $color_terms = get_the_terms($variant_id, 'fs_color');
            if (!empty($color_terms)) {
                foreach ($color_terms as $term) {
                    $colors[$term->slug] = true;
                }
            }
        }
    
        /*
         * ============================================================
         * 7️⃣ MARCA / GENERO DESDE PRODUCTOS VÁLIDOS
         * ============================================================
         */
    
        foreach ($valid_product_ids as $product_id) {
    
            $brand_terms = get_the_terms($product_id, 'fs_marca');
            if (!empty($brand_terms)) {
                foreach ($brand_terms as $term) {
                    $brands[$term->slug] = $term->name;
                }
            }
    
            $gender_terms = get_the_terms($product_id, 'fs_genero');
            if (!empty($gender_terms)) {
                foreach ($gender_terms as $term) {
                    $generos[$term->slug] = $term->name;
                }
            }
        }
    
        $sizes = array_keys($sizes);
        sort($sizes, SORT_NATURAL);
    
        return [
            'talla'      => $sizes,
            'superficie' => array_keys($surfaces),
            'color'      => array_keys($colors),
            'marca'      => $brands,
            'genero'     => $generos,
            'price_min'  => $price_min,
            'price_max'  => $price_max,
        ];
    }

    /* ============================================================
     *  PRODUCT AGGREGATES
     * ============================================================ */
    private function get_product_aggregates(int $product_id): array
    {
        $min_price = null;
        $colors    = [];
    
        /*
         * ============================================================
         *  1️⃣ OBTENER VARIANTES POR PARENT (NO MÁS LIKE)
         * ============================================================
         */
        $variant_ids = get_posts([
            'post_type'      => 'fs_variante',
            'post_status'    => 'publish',
            'fields'         => 'ids',
            'posts_per_page' => -1,
            'no_found_rows'  => true,
            'post_parent'    => $product_id,
        ]);
    
        if (empty($variant_ids)) {
            return [null, 0];
        }
    
        /*
         * ============================================================
         *  2️⃣ COLORES DESDE VARIANTES
         * ============================================================
         */
        foreach ($variant_ids as $variant_id) {
    
            $variant_id = (int) $variant_id;
    
            $color_terms = wp_get_post_terms($variant_id, 'fs_color');
            if (!empty($color_terms) && !is_wp_error($color_terms)) {
                foreach ($color_terms as $term) {
                    if (!empty($term->slug)) {
                        $colors[$term->slug] = true;
                    }
                }
            }
        }
    
        /*
         * ============================================================
         *  3️⃣ OBTENER TODOS LOS VARIANT_GLOBAL_ID
         * ============================================================
         */
        $variant_global_ids = [];
    
        foreach ($variant_ids as $variant_id) {
            $global_id = (string) get_post_meta((int)$variant_id, 'fs_variant_id', true);
            if ($global_id !== '') {
                $variant_global_ids[] = $global_id;
            }
        }
    
        $variant_global_ids = array_unique($variant_global_ids);
    
        if (empty($variant_global_ids)) {
            return [null, count($colors)];
        }
    
        /*
         * ============================================================
         *  4️⃣ QUERY MASIVA DE OFERTAS
         * ============================================================
         */
        $offer_meta_query = [
            'relation' => 'AND',
            [
                'key'   => 'fs_variant_id',
                'value' => $variant_global_ids,
                'compare' => 'IN',
            ],
            [
                'key'   => 'fs_in_stock',
                'value' => '1',
            ],
        ];
        
        // TALLA ACTIVA
        if (!empty($filters['talla'])) {
        
            $talla = str_replace('.', '-', (string) $filters['talla']);
        
            $offer_meta_query[] = [
                'key'     => 'fs_size_eu',
                'value'   => $talla,
                'compare' => '=',
            ];
        }
        
        // PRECIO MAX
        if (!empty($filters['price_max'])) {
            $offer_meta_query[] = [
                'key'     => 'fs_price',
                'value'   => (float) $filters['price_max'],
                'compare' => '<=',
                'type'    => 'NUMERIC',
            ];
        }
        
        // PRECIO MIN
        if (!empty($filters['price_min'])) {
            $offer_meta_query[] = [
                'key'     => 'fs_price',
                'value'   => (float) $filters['price_min'],
                'compare' => '>=',
                'type'    => 'NUMERIC',
            ];
        }
        
        $offer_ids = get_posts([
            'post_type'              => 'fs_oferta',
            'post_status'            => 'publish',
            'fields'                 => 'ids',
            'posts_per_page'         => -1,
            'no_found_rows'          => true,
            'meta_query'             => $offer_meta_query,
            'update_post_meta_cache' => true,
        ]);
    
        if (empty($offer_ids)) {
            return [null, count($colors)];
        }
    
        /*
         * ============================================================
         *  5️⃣ CALCULAR PRECIO MÍNIMO
         * ============================================================
         */
        foreach ($offer_ids as $offer_id) {
    
            $offer_id = (int) $offer_id;
    
            $in_stock = get_post_meta($offer_id, 'fs_in_stock', true);
            if (in_array($in_stock, ['0', 0, false, 'false', '', null], true)) {
                continue;
            }
    
            $price_raw      = (string) get_post_meta($offer_id, 'fs_price', true);
            $price_sale_raw = (string) get_post_meta($offer_id, 'fs_price_sale', true);
    
            $price      = (float) str_replace(',', '.', $price_raw);
            $price_sale = (float) str_replace(',', '.', $price_sale_raw);
    
            $final = ($price_sale > 0) ? $price_sale : $price;
    
            if ($final > 0) {
                $min_price = ($min_price === null)
                    ? $final
                    : min($min_price, $final);
            }
        }
    
        return [$min_price, count($colors)];
    }

    private function get_brand(int $id): ?string
    {
        $terms = wp_get_post_terms($id, 'fs_marca');
        return (!empty($terms) && !is_wp_error($terms))
            ? $terms[0]->name
            : null;
    }


    /* ============================================================
     *  WIZARD (VARIANTE + OFERTA DRIVEN)
     * ============================================================ */

    public function wizard_search(array $filters): array
    {
        $tax_query = ['relation' => 'AND'];

        // GÉNERO
        if (!empty($filters['gender'])) {

            $gender = sanitize_title($filters['gender']);

            if ($gender === 'infantil') {
                $tax_query[] = [
                    'taxonomy' => 'fs_age_group',
                    'field'    => 'slug',
                    'terms'    => 'kids',
                ];
            } else {
                $tax_query[] = [
                    'taxonomy' => 'fs_genero',
                    'field'    => 'slug',
                    'terms'    => $gender,
                ];
            }
        }

        // SUPERFICIE
        if (!empty($filters['surface'])) {

            $surface = sanitize_title($filters['surface']);

            if ($surface === 'turf') {

                $tax_query[] = [
                    'taxonomy' => 'fs_superficie',
                    'field'    => 'slug',
                    'terms'    => ['turf'],
                ];

            } elseif ($surface === 'indoor') {

                $tax_query[] = [
                    'taxonomy' => 'fs_superficie',
                    'field'    => 'slug',
                    'terms'    => ['indoor', 'mixta'],
                    'operator' => 'IN',
                ];

            } elseif ($surface === 'outdoor') {

                $tax_query[] = [
                    'taxonomy' => 'fs_superficie',
                    'field'    => 'slug',
                    'terms'    => ['outdoor', 'mixta'],
                    'operator' => 'IN',
                ];
            }
        }

        // CIERRE
        if (!empty($filters['closure'])) {
            $tax_query[] = [
                'taxonomy' => 'fs_cierre',
                'field'    => 'slug',
                'terms'    => sanitize_title($filters['closure']),
            ];
        }

        $variant_args = [
            'post_type'      => 'fs_variante',
            'post_status'    => 'publish',
            'posts_per_page' => 300,
            'fields'         => 'ids',
            'no_found_rows'  => true,
        ];

        if (count($tax_query) > 1) {
            $variant_args['tax_query'] = $tax_query;
        }

        $variant_query = new WP_Query($variant_args);

        if (empty($variant_query->posts)) {
            return [];
        }

        [$minBudget, $maxBudget] = $this->wizard_budget_to_range($filters['budget'] ?? '');

        $products = [];

        foreach ($variant_query->posts as $variant_id) {

            $product_id = (int) wp_get_post_parent_id($variant_id);
            if ($product_id <= 0) continue;

            $variant_global_id = get_post_meta($variant_id, 'fs_variant_id', true);
            if (empty($variant_global_id)) continue;

            $offer_query = new WP_Query([
                'post_type'      => 'fs_oferta',
                'post_status'    => 'publish',
                'posts_per_page' => -1,
                'fields'         => 'ids',
                'no_found_rows'  => true,
                'meta_query'     => [
                    [
                        'key'     => 'fs_variant_id',
                        'value'   => $variant_global_id,
                        'compare' => '=',
                    ],
                ],
            ]);

            if (empty($offer_query->posts)) continue;

            foreach ($offer_query->posts as $offer_id) {

                $in_stock = get_post_meta($offer_id, 'fs_in_stock', true);
                if (!$in_stock || $in_stock === '0' || $in_stock === 'false') continue;

                $price      = (float) str_replace(',', '.', (string) get_post_meta($offer_id, 'fs_price', true));
                $price_sale = (float) str_replace(',', '.', (string) get_post_meta($offer_id, 'fs_price_sale', true));

                $final_price = $price_sale > 0 ? $price_sale : $price;
                if ($final_price <= 0) continue;

                if ($minBudget !== null && $final_price < $minBudget) continue;
                if ($maxBudget !== null && $final_price > $maxBudget) continue;

                if (!isset($products[$product_id]) || $final_price < $products[$product_id]['price']) {
                    $products[$product_id] = [
                        'price'      => $final_price,
                        'variant_id' => $variant_id,
                        'offer_id'   => $offer_id,
                    ];
                }
            }
        }

        if (empty($products)) return [];

        uasort($products, fn($a, $b) => $a['price'] <=> $b['price']);
        $products = array_slice($products, 0, 3, true);

        $out = [];

        foreach ($products as $product_id => $data) {

            $variant_id = $data['variant_id'];
            $variant_image = null;

            $images_raw = get_post_meta($variant_id, 'fs_images', true);

            if (!empty($images_raw)) {
                $images_array = preg_split('/(?=https:\/\/)/', $images_raw, -1, PREG_SPLIT_NO_EMPTY);
                if (!empty($images_array[0])) {
                    $variant_image = trim($images_array[0]);
                }
            }

            $out[] = [
                'id'         => $product_id,
                'variant_id' => $variant_id,
                'offer_id'   => $data['offer_id'],
                'title'      => get_the_title($product_id),
                'link'       => get_permalink($product_id) . '?variant=' . $variant_id,
                'image'      => $variant_image,
                'price'      => $data['price'],
            ];
        }

        return $out;
    }

    private function wizard_budget_to_range(string $budget): array
    {
        $budget = strtolower(trim($budget));

        return match ($budget) {
            'low'  => [null, 60],
            'mid'  => [60, 100],
            'high' => [100, null],
            default => [null, null],
        };
    }
}